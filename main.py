...
class calculator:
    def streamlit_config(self):
        ...
        # page configuration
        st.set_page_config(page_title='HandyMath', layout="wide")
        ...
        # title and position
        st.markdown(f'<h1 style="text-align: center;"><i>HandyMath:</i> Virtual Calculator</h1>',
        ...
        st.markdown(
            """
            ### Instructions:
            * **Thumb + Index**: Draw lines on the canvas.
            * **Thumb + Middle**: Erase lines.
            * **Thumb + Pinky**: Reset the canvas.
            * **Index + Middle**: Analyze the equation.
            """,
            unsafe_allow_html=True,
        )
        add_vertical_space(1)
    ...
    def __init__(self):
        ...
        # Hand Preference (Default: Right-handed)
        self.hand_preference = "Right-handed"
    ...
    def identify_fingers(self):
        # Clear the fingers list before identification
        self.fingers = []
        ...
        if self.landmark_list:
            for id in [4, 8, 12, 16, 20]:  # Thumb, Index, Middle, Ring, Pinky
                if id != 4:
                    ...
                    if self.hand_preference == "Right-handed":
                        is_open = self.landmark_list[id][2] < self.landmark_list[id - 2][2]
                    else:  # Left-handed
                        is_open = self.landmark_list[id][2] < self.landmark_list[id - 2][2]
                    self.fingers.append(1 if is_open else 0)
                else:
                    # For the thumb
                    if self.hand_preference == "Right-handed":
                        is_open = self.landmark_list[id][1] < self.landmark_list[id - 2][1]
                    else:  # Left-handed
                        is_open = self.landmark_list[id][1] > self.landmark_list[id - 2][1]
                    self.fingers.append(1 if is_open else 0)

        # print(f"Fingers detected: {self.fingers} | Hand: {self.hand_preference}")
    ...
    def analyze_image_with_genai(self):
        ...
        prompt = "Analyze the image and provide the following:\n" \
                 "* The pure mathematical equation represented in the image.\n" \
    ...
    def main(self):
        ...
        with col3:
            self.hand_preference = st.radio("Hand Preference", options=["Right-handed", "Left-handed"])
